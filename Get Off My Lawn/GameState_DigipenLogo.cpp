...
\file		GameState_DigipenLogo.cpp 
...
#include "GameState_DigipenLogo.h" // header files
...
*/
/******************************************************************************/
int logo_display_time = 0; //counter for how long logo remains on screen
void GameStateDigipenLogoLoad(void)
{
	Spawn_Object(DIGIPEN_LOGO_BACKGROUND, 0.0f, 0.0f); // spawns background image
}
...
{
	logic_components.object_manager->Initiate_Fade_Black_Screen(false); //modifies transparency of black box to simulate fading in and out

	AEGfxSetBlendMode(AE_GFX_BM_BLEND); //Sets blend mode for background
}
...
void GameStateDigipenLogoUpdate(void)
{
	...
	++logo_display_time; //increases the counter for logo
	
	// Once time is hit, changes game state, starts the fade
	if (logo_display_time > 200)
	{
		gAEGameStateNext = GS_TEAMLOGO;
